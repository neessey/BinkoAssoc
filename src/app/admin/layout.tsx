...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        // Écoute en temps réel des nouvelles insertions
    ...
    const NavLink = ({ item, onClick }: { item: typeof navItems[0]; onClick?: () => void }) => {
        ...
        return (
            ...
            >
                ...
                {!collapsed && (
                    <>
                        <span className="text-xs tracking-wider uppercase font-medium flex-1">{item.label}</span>
                        {item.badge > 0 && (
                            <span className={cn(
                        ...
                        {active && item.badge === 0 && <ChevronRight className="w-3 h-3 ml-auto" />}
                    </>
                )}
                {/* Badge réduit quand sidebar collapsed */}
                ...
                <div className="h-18 flex items-center justify-between px-6 border-b border-border shrink-0">
                    {!collapsed && (
                        <div className="flex flex-col leading-tight">
                            <span className="text-xl font-serif font-semibold text-foreground tracking-wide">BINKO</span>
                            <span className="text-red text-[9px] tracking-[0.3em] uppercase font-sans">Admin Panel</span>
                        </div>
                    )}
                    <button
                ...
                <div className="p-3 border-t border-border">
                    <div className={cn(
                        ...
                        <div className="w-8 h-8 bg-red flex items-center justify-center shrink-0">
                            <span className="text-white text-xs font-serif font-semibold">A</span>
                        </div>
                        {!collapsed && (
                            <div className="flex-1 min-w-0">
                                <p className="text-xs text-foreground font-medium truncate">Administrateur</p>
                                <p className="text-xs text-muted-foreground truncate">admin@binko.ci</p>
                            </div>
                        )}
                    </div>
                    <Link
                        href="/"
                        className={cn(
                            "flex items-center gap-3 px-3 py-2 text-muted-foreground hover:text-red transition-colors mt-1",
                            collapsed ? "justify-center" : ""
                        )}
                    >
                        <LogOut className="w-4 h-4 shrink-0" />
                        {!collapsed && <span className="text-xs uppercase tracking-wider">Quitter</span>}
                    </Link>
                </div>
            </aside>
                ...
                <div className="lg:hidden fixed inset-0 z-50 flex">
                    ...
                    <aside className="relative w-64 bg-white flex flex-col border-r border-border z-10">
                        <div className="h-18 flex items-center justify-between px-6 border-b border-border">
                            <div className="flex flex-col leading-tight">
                                <span className="text-xl font-serif font-semibold text-foreground">BINKO</span>
                                <span className="text-red text-[9px] tracking-[0.3em] uppercase">Admin Panel</span>
                            </div>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <header className="h-18 bg-white border-b border-border flex items-center justify-between px-6 shrink-0">
                    ...
                    <div className="flex items-center gap-4 p-2">
                        {/* Cloche — point rouge seulement si ya du nouveau */}
